...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	DWORD dwSendRet = LVDS_Send(LVDS_CARD_NO,LVDS_DATA_CH1,byBuf,sizeof(byBuf)/sizeof(byBuf[0]),LVDS_WAIT_INFINITE);//超时等待,根据实际需要设置,LVDS_WAIT_INFINITE为一直等待

	/*********************下面演示RAM发送数据(连续发送)*********************/
	if (LVDS_IsSupportRAM(LVDS_CARD_NO))//判断是否支持RAM发送
	{
